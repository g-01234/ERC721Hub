...
contract ETFERC20 is ERC20, Spoke {
    ...
    constructor(
        ...
        uint256 _tokenId,
        address _calcNav
    ) payable ERC20(_name, _symbol, _decimals) Spoke(msg.sender, _tokenId) {
    ...
    function mintWithEth(uint256 amount) external payable {
        require(
            msg.value >=
                amount *
                    uint256(
                        ICalculateNAV(calcNav).calculateNAV(
                            trackedPriceFeeds,
                            tokens,
                            address(this)
                        )
    ...
    function mintWithRegisteredToken(
        uint256 amount,
        address token
    ) external payable {
        //insert logic here
